...
rms_audio_1 = librosa.feature.rms(Aud_1, frame_length=FRAME_SIZE, hop_length=HOP_LENGTH)[0]
rms_audio_2 = librosa.feature.rms(Aud_2, frame_length=FRAME_SIZE, hop_length=HOP_LENGTH)[0]

frames = range(len(rms_audio_1))
...
plt.plot(t, rms_audio_1, color="r")
plt.title("AUD_1")


plt.subplot(3, 1, 2)
librosa.display.waveshow(Aud_2, alpha=0.5)
plt.plot(t, rms_audio_2, color="r")
plt.title("AUD_2")
plt.show()

c = cr(rms_audio_1,rms_audio_2)
print(c)
print(len(c))
start = 0
